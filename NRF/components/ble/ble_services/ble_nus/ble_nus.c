...
#define BLE_UUID_NUS_BATT_CHARACTERISTIC 0x0004                     /**< The UUID of the BAT Characteristic. */
...
/**@brief Function for adding battery level characteristic.
 *
 * @param[in] p_nus       Nordic UART Service structure.
 * @param[in] p_nus_init  Information needed to initialize the service.
 *
 * @return NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t ble_add_caract_battery_level(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ble_gatts_char_md_t char_md;
	ble_gatts_attr_md_t cccd_md;
    ble_gatts_attr_t    attr_char_value;
    ...
    ble_uuid.type = p_nus->uuid_type;
    ble_uuid.uuid = BLE_UUID_NUS_BATT_CHARACTERISTIC;
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
	...
	// Add the BatteryLevel Characteristic.    
	err_code = ble_add_caract_battery_level(p_nus, p_nus_init);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_nus_battlvl_set(ble_nus_t * p_nus, uint8_t * p_string, uint16_t length)
{
    ble_gatts_hvx_params_t hvx_params;//Handle Value X
    ...
    hvx_params.handle = p_nus->batt_handles.value_handle;
    hvx_params.p_data = p_string;
    hvx_params.p_len  = &length;
    hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
